...
// ════════════════════════════════════════════════
// GET /api/products — aggregated ASIN performance
// ════════════════════════════════════════════════
productsRouter.get("/", async (req, res) => {
  try {
    ...
    let query = supabaseAdmin
      ...
      .select("asin, sku, campaign_id, campaign_name, ad_group_name, impressions, clicks, cost, orders, sales, add_to_cart")
      .eq("workspace_id", wsId)
      .gte("report_date", startDate)
      .lte("report_date", endDate);

    if (parsed.data.campaign_id) query = query.eq("campaign_id", parsed.data.campaign_id);

    const { data, error } = await query;
    if (error) throw error;

    // Aggregate per (asin + campaign + sku)
    const map = new Map<string, any>();
    for (const r of data || []) {
      const key = `${r.asin}||${r.campaign_id}||${r.sku || ""}`;
